    ...
    <div style={{ background: D.paper, fontFamily: D.sans, color: D.ink, minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1360, margin: '0 auto', padding: `32px ${padX}px ${isMobile ? 56 : 80}px` }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: 14 }}>
          {tiers.map((t, i) => (
            <div key={i} style={{ background: t.fav ? D.plum : D.card, color: t.fav ? D.paper : D.ink, borderRadius: 16, border: `1px solid ${t.fav ? D.plum : D.line}`, padding: isMobile ? 24 : 32 }}>
              <div style={{ fontFamily: D.mono, fontSize: 10, letterSpacing: 1, color: t.fav ? D.plumSoft : D.plum }}>{t.tier.toUpperCase()}</div>
              <div style={{ fontFamily: D.display, fontSize: isMobile ? 42 : 56, letterSpacing: -1.2, marginTop: 10, lineHeight: 1 }}>{t.price}</div>
