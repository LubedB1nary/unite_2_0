    ...
    <div style={{ background: D.paper, fontFamily: D.sans, color: D.ink, minHeight: '100vh' }}>
      ...
      <main id="main">
        ...
        <div style={{ maxWidth: 1360, margin: '0 auto', padding: '24px 40px 80px' }}>
          ...
          {featured && (
            <article onClick={() => navigate(`/blog/${featured.slug}`)} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 20, marginBottom: 32, background: D.card, border: `1px solid ${D.line}`, borderRadius: 16, overflow: 'hidden', cursor: 'pointer' }}>
              <PhotoPlaceholder caption={featured.cover} height={380} stripeFrom="#ebe3d3" stripeTo="#ddd1b7" textColor={D.plum} radius={0} />
              <div style={{ padding: 32, alignSelf: 'center' }}>
                ...
                <h2 style={{ fontFamily: D.display, fontSize: 44, fontWeight: 400, letterSpacing: -1, lineHeight: 1.05, marginTop: 14, color: D.ink }}>{featured.title}</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 18 }}>
            {rest.map((p) => (
              <article key={p.slug} onClick={() => navigate(`/blog/${p.slug}`)} style={{ background: D.card, borderRadius: 14, border: `1px solid ${D.line}`, overflow: 'hidden', cursor: 'pointer' }}>
                <PhotoPlaceholder caption={p.cover} height={180} stripeFrom="#ebe3d3" stripeTo="#ddd1b7" textColor={D.plum} radius={0} />
                <div style={{ padding: 22 }}>
                  ...
                  <h3 style={{ fontFamily: D.display, fontSize: 22, letterSpacing: -0.4, lineHeight: 1.2, marginTop: 10, minHeight: 76, fontWeight: 400, color: D.ink }}>{p.title}</h3>
                </div>
              </article>
            ))}
          </div>
