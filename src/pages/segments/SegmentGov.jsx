      ...
      <div style={{ maxWidth: 1360, margin: '0 auto', padding: `32px ${padX}px ${isMobile ? 56 : 80}px` }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: 12, marginBottom: 28 }}>
          {[['36C24123A0077', 'MSPV BPA'], ['VOSB', 'Veteran-owned'], ['Berry', 'Compliant PPE'], ['Buy America', 'Act eligible']].map(([b, s], i) => (
            <div key={i} style={{ padding: isMobile ? 18 : 22, background: D.card, borderRadius: 14, border: `1px solid ${D.line}` }}>
              <div style={{ fontFamily: D.display, fontSize: isMobile ? 20 : 28, color: D.plum, letterSpacing: -0.5, lineHeight: 1, wordBreak: 'break-word' }}>{b}</div>
              <div style={{ fontFamily: D.mono, fontSize: 10, letterSpacing: 1, color: D.ink3, marginTop: 10 }}>{s.toUpperCase()}</div>
            </div>
          ))}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1fr', gap: isMobile ? 22 : 32, alignItems: 'start' }}>
          <div>
            <div style={{ fontFamily: D.display, fontSize: 'clamp(28px, 4.8vw, 44px)', letterSpacing: -0.9, lineHeight: 1.08 }}>Procurement mechanisms we support.</div>
            <div style={{ marginTop: 20, display: 'grid', gap: 10 }}>
              {[['MSPV-NG', 'Medical/Surgical Prime Vendor, Next Generation · VA-wide'], ['SAM.gov', 'Active registration. Ready to bid RFQs, IFBs, and RFPs.'], ['GSA Advantage', 'Catalog SKUs available via federal purchase card.'], ['Punch-out', 'cXML / OCI integration with VA procurement systems.'], ['Set-asides', 'SDVOSB and VOSB set-aside preferences honored.']].map(([h, s]) => (
                <div key={h} style={{ padding: isMobile ? 16 : 18, background: D.card, borderRadius: 12, border: `1px solid ${D.line}`, display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '160px 1fr', gap: isMobile ? 6 : 20 }}>
          ...
          <div style={{ padding: isMobile ? 22 : 28, background: D.plum, color: D.paper, borderRadius: 16 }}>
